...
INNER JOIN address ON customer.shipping_address_id = address.id
WHERE work.id = ?';
...
function generate_work_popout_info($work_id) {
  ...
  if ($result->num_rows>0) {
    ...
    $work_info_output = sprintf($work_info_html, $work_title, $work_description, ($work_start.' - '.$work_end), $work_type_name, $work_type_description, $customer_name, $contact, $address, $employee_outupt);
  }
  echo $work_info_output;
}
